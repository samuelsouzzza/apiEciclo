...
async function updatePublication(request, response) {
  ...
  if (
    user &&
    !user.publication.some((pub) => pub._id.toString() === idPublication)
  ) {
    return response
      .status(403)
      .json({ response: 'Publicação não pertence ao usuário.' });
  }

  return response
    .status(200)
    .json({ response: 'Atualização da publicação bem-sucedida' });
}
